...
class NostoJobSyncService
{
    ...
    public function deleteRunningFullProductSyncJobs(Context $context, string $handlerCode): void
    {
        ...
        if (empty($jobIds)) {
            throw new Exception('There are currently no jobs available for cancellation, all jobs are completed successfully.');
        }
        ...
        try {
            ...
            $deleteIds = array_map(fn($id) => ['id' => $id], array_values($jobIds));
